<template>
  <div id="add-blog" class="container">
    <h3 align="center"><i class="el-icon-edit"></i></h3>
    <form>
      <!--博客标题-->
    ...
    <div id="preview">
      <div><h2 align="center"><i class="el-icon-document"></i></h2></div>
      <h5> 标题: </h5>
      <p>{{blog.title}}</p>
      <h5> 内容:</h5>
      <p>{{blog.content}}</p>
...
<script>
  ...
  export default {
    data() {
      return {
        blog: {
          ...
          content: '',
          category: [],
          author: 'longjian'  //设置 select 的默认值
        },
    ...
    methods: {
      post() {
        // 判断输入合法性
        const {title, content, category} = this.blog
        if (!title) {
          this.$message({
            showClose: true,
            message: '请输入标题',
          ...
          // 使用post,提交数据
          axios.post('https://vueblogs-bca09.firebaseio.com/posts.json', this.blog)  //直接post blog对象到网址中
            .then((res) => {
              this.$router.push({path:'/show',query:{alert:'添加成功！'}})
            })
        }
      }
    },
